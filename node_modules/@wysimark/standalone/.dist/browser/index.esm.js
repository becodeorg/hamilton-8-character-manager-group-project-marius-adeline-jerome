...
init_react_shim();
import { Slate as Slate2 } from "slate-react";
...
function createEditable(plugins2) {
  ...
  return function SinkEditable3(props) {
    let CurrentRenderEditable = (props2) => /* @__PURE__ */ jsx(Editable, { ...props2 });
    for (const fn of fns) {
      const PrevRenderEditable = CurrentRenderEditable;
      CurrentRenderEditable = (props2) => {
        return fn({
          attributes: props2,
    ...
    return /* @__PURE__ */ jsx(CurrentRenderEditable, { ...props });
  };
}
...
function createRenderElement(originalFn, plugins2) {
  ...
  return function(renderElementProps) {
    for (const fn of fns) {
...
function SinkEditable(originalProps) {
  ...
  const NextEditable = createEditable(plugins2);
  return /* @__PURE__ */ jsx2(NextEditable, { ...originalProps, ...nextProps });
}
...
function standardizeNodeMatcher(matchNode) {
  ...
  if (typeof matchNode === "string")
    return (node2) => Element2.isElement(node2) && node2.type === matchNode;
  if (Array.isArray(matchNode))
    return (node2) => Element2.isElement(node2) && matchNode.includes(node2.type);
  throw new Error(
...
function isElementType(node2, type) {
  if (Array.isArray(type)) {
    return Element4.isElement(node2) && type.includes(node2.type);
  }
  if (typeof type === "string") {
    return Element4.isElement(node2) && node2.type === type;
  }
  throw new Error(
...
function createIsElementType(type) {
  if (Array.isArray(type)) {
    return (node2) => Element4.isElement(node2) && type.includes(node2.type);
  } else {
    return (node2) => Element4.isElement(node2) && type == node2.type;
  }
}
...
import { Editor as Editor7, Element as SlateElement, Range as Range2, Transforms } from "slate";
  ...
  return (e) => {
    ...
    const convertibleBlockEntry = findElementUp(
      ...
      (node2) => (
         ...
         */
        SlateElement.isElement(node2) && editor.convertElement.isConvertibleElement(node2)
      )
    );
...
init_react_shim();
import { Editor as Editor11, Element as Element5, Path as Path4, Transforms as Transforms4 } from "slate";
function insertRootElement(editor, element2, { at = editor.selection } = {}) {
  if (at == null)
    return false;
  const entry = findElementUp(
    editor,
    (node2) => Element5.isElement(node2) && editor.isMaster(node2)
  );
  if (entry == null) {
...
init_react_shim();
import { Editor as Editor15, Range as Range3, Text, Transforms as Transforms8 } from "slate";
function insertLink(editor, href, text4 = href, { select = true } = {}) {
  ...
  if (Range3.isCollapsed(selection)) {
    ...
    Transforms8.wrapNodes(
      ...
      {
        split: true,
        match: (node2) => Text.isText(node2) || Editor15.isInline(editor, node2)
      }
    );
  }
}
...
init_react_shim();
import { Element as Element7, Transforms as Transforms10 } from "slate";
function normalizeNode(editor, entry) {
  if (!Element7.isElement(entry[0]))
    return false;
  ...
  for (let i = 0; i < children.length; i++) {
    ...
    if (!Element7.isElement(child) || child.type !== "anchor")
      continue;
...
function Menu({
  ...
  return /* @__PURE__ */ jsxs3(Fragment2, { children: [
    ...
    /* @__PURE__ */ jsx7($Menu, { ref, style, children: items2.map((item, index2) => {
      if (item === "divider") {
        return /* @__PURE__ */ jsx7($MenuDivider, {}, index2);
      } else {
        return /* @__PURE__ */ jsx7(MenuItem, { editor, item, close }, index2);
      }
    }) })
  ] });
}
...
function AnchorDialog({
  ...
  const url = parseUrl(element2.href);
  const style = useAbsoluteReposition(
    { destAnchor, destStartEdge },
    ({ destAnchor: destAnchor2, destStartEdge: destStartEdge2 }) => {
      return {
      ...
      "a",
      {
        ...
        children: [
          ...
          /* @__PURE__ */ jsxs6("div", { className: "--url", children: [
            ...
            /* @__PURE__ */ jsx13("div", { className: "--pathname", children: url.pathname })
          ] })
        ]
      }
    ),
...
init_react_shim();
import { Element as Element8, Path as Path6 } from "slate";
function isSafeDelete(editor, a, b) {
  ...
  const masterEntryA = findElementUp(
    editor,
    (el) => Element8.isElement(el) && editor.isMaster(el),
    { at: a[1] }
  );
  const masterEntryB = findElementUp(
    editor,
    (el) => {
      return Element8.isElement(el) && editor.isMaster(el);
    },
    { at: b[1] }
  );
...
init_react_shim();
import { Element as Element9, Transforms as Transforms12 } from "slate";
...
function matchBlockQuoteSafe(node2) {
  return Element9.isElement(node2) && /**
   * TODO:
   *
   * This is probably:
    ...
    return {
      ...
      editor: {
        normalizeNode(entry) {
          ...
          if (!Element9.isElement(node2))
            return false;
          if (node2.type !== "block-quote")
            return false;
          return normalizeSiblings(editor, [node2, path3], (a, b) => {
            if (Element9.isElement(a[0]) && Element9.isElement(b[0]) && a[0].type === "block-quote" && b[0].type === "block-quote") {
...
init_react_shim();
import { Element as Element13, Transforms as Transforms15 } from "slate";

// ../react/src/code-block-plugin/decorate.tsx
init_react_shim();
import { languages, tokenize } from "prismjs";
import { Element as Element10, Node as Node6 } from "slate";
...
init_react_shim();
import { Element as Element11, Transforms as Transforms13 } from "slate";
function setCodeBlockLanguage(editor, language, options = {}) {
  const entry = findElementUp(
    editor,
    (el) => Element11.isElement(el) && el.type === "code-block",
    { at: options.at }
  );
  if (!entry)
    return false;
...
init_react_shim();
import { Element as Element12, Node as Node7, Transforms as Transforms14 } from "slate";
function normalizeNode2(editor, entry) {
  if (!Element12.isElement(entry[0]))
    return false;
  if (entry[0].type === "code-block-line") {
    for (const [child, path3] of Node7.children(editor, entry[1])) {
      if (!Element12.isElement(child))
        continue;
...
function CodeBlock({
  ...
  const onClick = useCallback5(() => {
    ...
    if (dest === null)
      return;
    const items2 = LanguageList.map((language) => {
      return {
    ...
    dropdown.open(() => /* @__PURE__ */ jsx19(Menu, { dest, items: items2, close: dropdown.close }));
  }, [element2]);
...
init_react_shim();
import { Element as Element16 } from "slate";

// ../react/src/collapsible-paragraph-plugin/normalize-node/normalize-sibling-paragraphs.ts
init_react_shim();
import { Element as Element14, Transforms as Transforms16 } from "slate";
function isParagraph(node2) {
  return Element14.isElement(node2) && node2.type === "paragraph";
}
...
init_react_shim();
import { Element as Element15, Transforms as Transforms17 } from "slate";
function isWall(editor, node2) {
  if (!Element15.isElement(node2))
    return false;
...
init_react_shim();
import { Editor as Editor25, Element as Element18 } from "slate";
function convertElements(editor, matchForToggle, targetElement, allowToggle) {
  const entries = Array.from(
    Editor25.nodes(editor, {
      match: (node2) => Element18.isElement(node2) && editor.convertElement.isConvertibleElement(node2)
    })
  );
  if (entries.length === 0)
    return false;
  ...
  if (shouldToggle) {
    Editor25.withoutNormalizing(editor, () => {
      for (const entry of entries) {
...
init_react_shim();
import { Editor as Editor26, Path as Path8, Range as Range5, Transforms as Transforms18 } from "slate";
function insertBreak(editor) {
  ...
  if (Range5.isExpanded(editor.selection))
    return false;
  if (!Editor26.isEnd(editor, editor.selection.anchor, entry[1]))
    return false;
  ...
  Transforms18.select(editor, {
    anchor: Editor26.start(editor, nextPath),
    focus: Editor26.start(editor, nextPath)
  });
  return true;
}
...
init_react_shim();
import { Editor as Editor30, Transforms as Transforms23 } from "slate";
...
init_react_shim();
import { Editor as Editor28, Transforms as Transforms21 } from "slate";
...
function convertToInlineImage(editor, element2) {
  ...
  Editor28.withoutNormalizing(editor, () => {
...
init_react_shim();
import { Editor as Editor29, Text as Text2, Transforms as Transforms22 } from "slate";
...
function convertToBlockImage(editor, element2) {
  ...
  Editor29.withoutNormalizing(editor, () => {
    ...
    const parentEntry = findElementUp(
      editor,
      (node2) => Editor29.isBlock(editor, node2) && node2.type !== "image-block"
    );
    if (!parentEntry)
    ...
    if (index2 === siblingCount - 2 && Text2.isText(lastSibling) && lastSibling.text === "") {
      Transforms22.removeNodes(editor, {
        at: [...parentPath, siblingCount - 1]
      });
    }
    const firstSibling = siblings[0];
    const removeFirstSibling = index2 === 1 && Text2.isText(firstSibling) && firstSibling.text === "";
  ...
  createPlugin(
    (editor, sinkOptions, { createPolicy }) => {
      ...
      editor.upload.onUploadImageFile = (e) => {
        ...
        if (!selection) {
          const lastPos = Editor30.end(editor, []);
...
init_react_shim();
import { Editor as Editor31, Transforms as Transforms24 } from "slate";
function insertBreak2(editor) {
  const entry = findElementUp(editor, isListItem);
  if (!entry)
    return false;
  const [element2, path3] = entry;
  if (Editor31.isEmpty(editor, element2)) {
    if (element2.depth > 0) {
...
init_react_shim();
import { Element as Element19, Transforms as Transforms25 } from "slate";
function normalizeOrderedFirstAtDepth(editor, entry) {
  const [node2, path3] = entry;
  if (!Element19.isElement(node2))
    return false;
  return normalizeSiblings(editor, [node2, path3], (a, b) => {
    if (!isListItem(a[0]) || !isElementType(b[0], "ordered-list-item"))
      return false;
    const __firstAtDepth = b[0].depth > a[0].depth;
    if (b[0].__firstAtDepth !== __firstAtDepth) {
      Transforms25.setNodes(editor, { __firstAtDepth }, { at: b[1] });
      return true;
    }
    return false;
  });
...
function TaskListItem({
  ...
  return /* @__PURE__ */ jsxs20($TaskListItem, { ...attributes, style, children: [
    /* @__PURE__ */ jsx43("div", { className: "--list-item-icon", contentEditable: false, children: element2.checked ? /* @__PURE__ */ jsx43(CheckedIcon, {}) : /* @__PURE__ */ jsx43(UncheckedIcon, {}) }),
    children
  ] });
}
  ...
  (editor, options, { createPolicy }) => {
    ...
    return createPolicy({
      ...
      editableProps: {
        ...
        onKeyDown(e) {
          if (!Editor34.nodes(editor, { match: isListItem }))
            return false;
...
init_react_shim();
import { Editor as Editor37, Range as Range6, Transforms as Transforms26 } from "slate";
...
function getRangeBackwards(editor, point3, startDistance, endDistance) {
  const startPoint = Editor35.before(editor, point3, {
    unit: "character",
    distance: startDistance
  });
  const endPoint = endDistance === void 0 ? point3 : Editor35.before(editor, point3, {
    unit: "character",
...
init_react_shim();
import { Editor as Editor36, Element as Element20 } from "slate";
function getTextBeforePointInBlock(editor, point3) {
  const blockEntry = findElementUp(
    editor,
    (node2) => Element20.isElement(node2) && !Editor36.isVoid(editor, node2) && Editor36.isBlock(editor, node2)
  );
  if (blockEntry == null)
    return null;
  const range = {
    anchor: Editor36.start(editor, blockEntry[1]),
    focus: point3
  };
  return Editor36.string(editor, range);
}
...
function autocompleteMark(editor, text4, {
  ...
  return () => {
    ...
    Transforms26.delete(editor, { at: closingMarkersRange });
    ...
    Transforms26.delete(editor, { at: openingMarkersRange });
    ...
    Editor37.removeMark(editor, mark);
  };
}
...
init_react_shim();
import { Editor as Editor38, Text as Text4, Transforms as Transforms27 } from "slate";
function toggleMark(editor, markKey, unsetKey, { at = editor.selection } = {}) {
  if (at == null)
    return;
  const [match] = Editor38.nodes(editor, {
    match: (n) => Text4.isText(n) && !!n[markKey],
    at
  });
  ...
  if (typeof unsetKey === "string") {
    Transforms27.unsetNodes(editor, unsetKey, {
      match: (n) => Text4.isText(n),
      split: true,
      at
    });
...
init_react_shim();
import { Editor as Editor39, Point as Point3 } from "slate";
function forceNormalizeNearestElement(editor) {
  if (!editor.selection)
    return;
  const entry = Editor39.parent(editor, editor.selection);
...
init_react_shim();
import { Element as Element22 } from "slate";

// ../react/src/table-plugin/delete-fragment/index.ts
init_react_shim();
import { Editor as Editor41, Path as Path10, Transforms as Transforms28 } from "slate";
...
function getReversedDeleteSafeRanges(editor, deleteRange, protectedTypes) {
  const positions = [...Editor40.positions(editor, { at: deleteRange })];
  ...
  for (const pos of positions) {
    ...
    if (startTdPath && tdPath && Path9.equals(startTdPath, tdPath) || startTdPath == void 0 && tdPath == void 0) {
      prevPos = pos;
    } else {
...
function deleteFragmentWithProtectedTypes(editor, protectedTypes) {
  if (editor.selection == null)
    return false;
  const [start, end] = Editor41.edges(editor, editor.selection);
  ...
  if (startProtectedPath && endProtectedPath && Path10.equals(startProtectedPath, endProtectedPath)) {
    return false;
  }
  ...
  Editor41.withoutNormalizing(editor, () => {
    for (const range of reversedRanges) {
      Transforms28.delete(editor, { at: range });
    }
    Transforms28.collapse(editor, { edge: "start" });
  });
  return true;
}
...
init_react_shim();
import { Editor as Editor42, Transforms as Transforms29 } from "slate";
...
function insertColumn(editor, { offset = 0, at = editor.selection } = {}) {
  ...
  Editor42.withoutNormalizing(editor, () => {
    const { columns } = tableElement;
    const nextColumns = [...columns];
    nextColumns.splice(nextCellIndex, 0, columns[nextCellIndex]);
    Transforms29.setNodes(editor, { columns: nextColumns }, { at: tablePath });
    tableElement.children.forEach((rowElement, i) => {
      Transforms29.insertNodes(editor, createCell(nextCellIndex), {
...
init_react_shim();
import { Transforms as Transforms30 } from "slate";
...
function insertRow(editor, { at = editor.selection, offset = 0 } = {}) {
  ...
  Transforms30.insertNodes(editor, nextRowElement, {
...
init_react_shim();
import { Editor as Editor44, Element as Element21, Path as Path11, Transforms as Transforms31 } from "slate";
...
function insertRootElement2(editor, element2, { at = editor.selection } = {}) {
  ...
  const entry = findElementUp(
    editor,
    (node2) => Element21.isElement(node2) && editor.isMaster(node2)
  );
  if (entry == null) {
    const selection = editor.selection;
    Editor44.withoutNormalizing(editor, () => {
      Transforms31.insertNodes(editor, element2, { at });
      if (selection) {
        Transforms31.select(editor, selection);
        Transforms31.move(editor);
      }
    });
  } else {
    const nextPath = Path11.next(entry[1]);
    Editor44.withoutNormalizing(editor, () => {
      Transforms31.insertNodes(editor, element2, { at: nextPath });
      Transforms31.select(editor, Editor44.start(editor, nextPath));
    });
  }
  return true;
}
...
function selectElementBelow(editor, t) {
  ...
  try {
    selectStartOfElement(editor, Path12.next(tablePath));
    return true;
  } catch (e) {
    return false;
  }
...
function selectElementAbove(editor, t) {
  ...
  try {
    selectEndOfElement(editor, Path12.previous(tablePath));
    return true;
  } catch (e) {
    return false;
  }
...
function removeTable(editor) {
  ...
  if (t === void 0)
    return false;
  Transforms32.removeNodes(editor, { at: t.tablePath });
  return true;
}
...
function removeColumn(editor, { at = editor.selection } = {}) {
  ...
  Editor47.withoutNormalizing(editor, () => {
    ...
    Transforms33.setNodes(editor, { columns }, { at: tablePath });
    tableElement.children.forEach((rowElement, rowIndex2) => {
      Transforms33.removeNodes(editor, {
        at: [...tablePath, rowIndex2, cellIndex]
      });
    });
    const selection = Editor47.start(editor, [
      ...tablePath,
      rowIndex,
...
init_react_shim();
import { Editor as Editor48, Transforms as Transforms34 } from "slate";
function removeRow(editor, { at = editor.selection } = {}) {
  ...
  Editor48.withoutNormalizing(editor, () => {
    Transforms34.removeNodes(editor, { at: t.rowPath });
    Transforms34.select(
      editor,
...
function setTableColumnAlign(editor, options) {
  ...
  Transforms35.setNodes(editor, { columns: nextColumns }, { at: tablePath });
  return true;
}
...
function selectCell(editor, { at = editor.selection } = {}) {
  ...
  if (t === void 0)
    return false;
  const { cellPath } = t;
  Transforms36.select(editor, cellPath);
  return true;
}
  ...
  rowElements.forEach((rowElement, y) => {
    ...
    cellElements.forEach((cellElement, x) => {
      if (cellElement.x !== x || cellElement.y !== y) {
        Transforms37.setNodes(editor, { x, y }, { at: [...entry[1], y, x] });
        isTransformed = true;
      }
    });
  });
...
init_react_shim();
import { Editor as Editor52, Transforms as Transforms38 } from "slate";
function normalizeTableCell(editor, entry) {
  ...
  Editor52.withoutNormalizing(editor, () => {
    ...
    for (let i = node2.children.length; i >= 0; i--) {
      Transforms38.mergeNodes(editor, { at: [...path3, i] });
    }
    Transforms38.delete(editor, {
...
init_react_shim();
import { useCallback as useCallback10, useRef as useRef7, useState as useState5 } from "react";
...
function ColumnMenu({ cellElement }) {
  ...
  const onMouseEnter = useCallback10(() => {
    setHover(true);
  }, []);
  const onMouseLeave = useCallback10(() => {
    setHover(false);
  }, []);
  const onClick = useCallback10(() => {
    ...
    menu.open(() => /* @__PURE__ */ jsx49(Menu, { dest, items: items2, close: menu.close }));
  }, []);
...
import { useFocused, useSlateStatic as useSlateStatic22 } from "slate-react";
...
import { useCallback as useCallback11, useRef as useRef8, useState as useState7 } from "react";
...
function TableDialog({
  ...
  const hoverCell = useCallback11(
    (x, y) => {
      setHover({ x, y });
    },
    [setHover]
  );
  const createTable2 = useCallback11(
    (x, y) => {
...
var More = () => /* @__PURE__ */ jsx58(TablerIcon, { width: "0.5em", viewBox: "0 0 12 24", children: /* @__PURE__ */ jsx58("path", { d: "m2 12 4 4 4-4", opacity: 0.375 }) });
...
var Attachment = () => /* @__PURE__ */ jsx58(TablerIcon, { children: /* @__PURE__ */ jsx58("path", { d: "m15 7-6.5 6.5a1.5 1.5 0 0 0 3 3L18 10a3 3 0 0 0-6-6l-6.5 6.5a4.5 4.5 0 0 0 9 9L21 13" }) });
...
function AnchorDialog2({
  ...
  const onChangeInput = useCallback12(
    (e) => {
  ...
  return /* @__PURE__ */ jsxs28(Fragment8, { children: [
    ...
    /* @__PURE__ */ jsxs28($AnchorDialog, { ref, style, children: [
      /* @__PURE__ */ jsxs28($AnchorDialogInputLine, { children: [
        /* @__PURE__ */ jsx59(
          $AnchorDialogInput,
          {
            type: "text",
        ...
        /* @__PURE__ */ jsxs28($DialogButton, { onClick: insertLink2, children: [
          /* @__PURE__ */ jsx59(Link, {}),
...
function EmojiDialog({
  ...
  const onEmojiSelect = useCallback13(
    (e) => {
      Transforms39.insertText(editor, e.native);
  ...
  return /* @__PURE__ */ jsxs29(Fragment9, { children: [
    ...
    /* @__PURE__ */ jsx60($EmojiDialog, { ref, style, children: /* @__PURE__ */ jsx60(Picker, { data, onEmojiSelect, autoFocus: true }) })
  ] });
}
var $EmojiDialog = styled35($Panel)``;
...
function FileDialog({
  ...
  return /* @__PURE__ */ jsxs30(Fragment10, { children: [
    ...
    /* @__PURE__ */ jsxs30($FileDialog, { ref, style, children: [
      /* @__PURE__ */ jsxs30("label", { children: [
        ...
        /* @__PURE__ */ jsxs30($DialogButton, { children: [
          icon,
          /* @__PURE__ */ jsx61("span", { style: { marginLeft: "0.5em" }, children: buttonCaption })
        ] })
      ] }),
      /* @__PURE__ */ jsx61($DialogHint, { children: buttonHint })
    ] })
  ] });
}
function AttachmentDialog({
  ...
  return /* @__PURE__ */ jsx61(
    FileDialog,
    {
      dest,
      close,
      icon: /* @__PURE__ */ jsx61(FileUpload, {}),
...
// ../react/src/toolbar-plugin/items/heading-items.tsx
init_react_shim();
var headingItems = [
  {
    icon: H1,
...
// ../react/src/toolbar-plugin/items/style-items.tsx
init_react_shim();
var styleItems = [
  {
    icon: Superscript,
  ...
  "divider",
  { icon: RemoveStyles, title: "Remove Styles", hotkey: "super+0" }
];
...
var items = [
  ...
  { icon: Style, title: "Text Style", more: true, children: styleItems },
  "divider",
  {
    icon: BulletList,
    title: "Lists",
  ...
  "divider",
  { icon: Emoji, title: "Insert Emoji", Component: EmojiDialog }
];
...
import { ReactEditor as ReactEditor15, useSlateStatic as useSlateStatic20 } from "slate-react";
...
function ToolbarButton({
  ...
  const openMenu = useCallback14(() => {
    const dest = ref.current;
    const items2 = item.children;
    const Component = item.Component;
    if (!dest)
      return;
    if (items2) {
      menu.open(() => /* @__PURE__ */ jsx62(Menu, { dest, items: items2, close: menu.close }));
    } else if (Component) {
      menu.open(() => /* @__PURE__ */ jsx62(Component, { dest, close: menu.close }));
    }
  }, [item]);
...
function ToolbarItem({ item }) {
  ...
  if (item === "divider") {
    return /* @__PURE__ */ jsx63($ToolbarDivider, {});
  } else {
...
function Toolbar() {
  return /* @__PURE__ */ jsx63($Toolbar, { children: items.map((item, index2) => /* @__PURE__ */ jsx63(ToolbarItem, { item }, index2)) });
}
...
init_react_shim();
import { Editor as Editor53, Node as Node12, Path as Path13, Transforms as Transforms40 } from "slate";
    ...
    return {
      ...
      editor: {
        normalizeNode: (entry) => {
          ...
          if (Editor53.hasBlocks(editor, lastElement) || Editor53.isVoid(editor, lastElement)) {
            Transforms40.insertNodes(
              editor,
              { type: "paragraph", children: [{ text: "" }] },
              {
                at: Path13.next(lastPath)
              }
            );
          }
          return true;
...
import {
  Editor as Editor54,
  Element as Element24,
  Transforms as Transforms41
} from "slate";
...
function setElementTimeTraveling(editor, prev, next) {
  ...
  const nodeEntries = [
    ...Editor54.nodes(editor, {
      mode: "all",
      at: [],
      match: (n) => Element24.isElement(n) && /**
  ...
  for (const nodeEntry of nodeEntries) {
    Transforms41.setNodes(editor, next, { at: nodeEntry[1] });
...
init_react_shim();
import { Editor as Editor55, Transforms as Transforms42 } from "slate";
...
var AttachmentPlugin = createPlugin(
  (editor, options, { createPolicy }) => {
    ...
    editor.upload.onUploadFile = ({ hashUrl, file }) => {
      const { selection } = editor;
      Transforms42.insertText(editor, `\u{1F517} `);
      Transforms42.insertNodes(editor, {
        type: "anchor",
        href: hashUrl,
        children: [{ text: file.name }]
      });
      if (!selection) {
        const lastPos = Editor55.end(editor, []);
        Transforms42.select(editor, lastPos);
        ReactEditor16.focus(editor);
      }
      Transforms42.move(editor, { distance: 1, unit: "offset" });
      return true;
    };
  ...
  function afterLabelEnd(code2) {
    ...
    if (code2 === 91) {
      return effects.attempt(
        ...
        ok2,
        defined2 ? effects.attempt(collapsedReferenceConstruct, ok2, balanced) : balanced
      )(code2);
    }
    return defined2 ? ok2(code2) : balanced(code2);
  }
...
function tokenizeDefinitionContinuation(effects, ok2, nok) {
  return effects.check(blankLine, ok2, effects.attempt(indent2, ok2, nok));
}
...
// ../convert/src/parse/parse-content.ts
init_react_shim();

// ../convert/src/utils.ts
init_react_shim();
...
// ../convert/src/parse/parse-footnote-definition.ts
init_react_shim();
...
// ../convert/src/parse/parse-heading.ts
init_react_shim();

// ../convert/src/parse/parse-phrasing-content/parse-phrasing-content.ts
init_react_shim();

// ../convert/src/parse/parse-phrasing-content/normalize-segments.ts
init_react_shim();
import { Text as SlateText3 } from "slate";

// ../convert/src/parse/parse-phrasing-content/normalize-segment.ts
init_react_shim();
import { Text as SlateText2 } from "slate";

// ../convert/src/serialize/serialize-line/utils/index.ts
init_react_shim();

// ../convert/src/serialize/serialize-line/utils/is-utils.ts
init_react_shim();
...
// ../convert/src/serialize/serialize-line/utils/mark-utils/index.ts
init_react_shim();

// ../convert/src/serialize/serialize-line/utils/mark-utils/mark-convert-utils.ts
init_react_shim();
...
// ../convert/src/serialize/serialize-line/utils/mark-utils/mark-get-utils.ts
init_react_shim();
...
// ../convert/src/serialize/serialize-line/utils/mark-utils/mark-order-utils.ts
init_react_shim();
...
// ../convert/src/serialize/serialize-line/utils/text-utils.ts
init_react_shim();
var ESCAPES = [
  "\\",
  // escape
...
// ../convert/src/parse/parse-phrasing-content/parse-inline-image/index.ts
init_react_shim();

// ../convert/src/parse/parse-phrasing-content/parse-inline-image/image-parsers.ts
init_react_shim();

// ../convert/src/parse/parse-phrasing-content/parse-inline-image/parse-generic-image.ts
init_react_shim();
...
// ../convert/src/parse/parse-phrasing-content/parse-inline-image/parse-portive-image.ts
init_react_shim();

// ../convert/src/parseUrl.ts
init_react_shim();
var URL_REGEX = /^(\/[^?#]*)(?:\?([^#]*))?(#.*)?$/;
...
// ../convert/src/parse/parse-phrasing-content/parse-inline-image/parse-utils.ts
init_react_shim();
...
// ../convert/src/parse/parse-phrasing-content/parse-inline-image/parse-uncommon-mark-image.ts
init_react_shim();
...
// ../convert/src/parse/parse-html.ts
init_react_shim();
...
// ../convert/src/parse/parse-list/index.ts
init_react_shim();

// ../convert/src/parse/parse-list/parse-list.ts
init_react_shim();

// ../convert/src/parse/parse-list/parse-list-item.ts
init_react_shim();

// ../convert/src/parse/parse-list/parse-list-item-child.ts
init_react_shim();
...
// ../convert/src/parse/parse-paragraph.ts
init_react_shim();
...
// ../convert/src/parse/parse-table.ts
init_react_shim();
...
// ../convert/src/parse/parse-thematic-break.ts
init_react_shim();
...
// ../convert/src/parse/transform-inline-links.ts
init_react_shim();
...
function transformInlineLinks(tree) {
  ...
  visit(tree, (node2, index2, parent) => {
    ...
    if (node2.type === "imageReference" || node2.type === "linkReference") {
      const def = definition2(node2.identifier);
...
// ../convert/src/serialize/index.ts
init_react_shim();

// ../convert/src/serialize/normalize/normalizeElementListDepths.ts
init_react_shim();
...
// ../convert/src/serialize/serialize-elements.ts
init_react_shim();

// ../convert/src/serialize/serialize-element.ts
init_react_shim();

// ../convert/src/serialize/serialize-code-block/index.ts
init_react_shim();

// ../convert/src/serialize/serialize-code-block/serialize-code-line.ts
init_react_shim();
...
// ../convert/src/serialize/serialize-image-block/index.ts
init_react_shim();

// ../convert/src/serialize/serialize-image-shared/index.ts
init_react_shim();

// ../convert/src/serialize/serialize-image-shared/serialize-generic-image-url.ts
init_react_shim();
function serializeGenericImageUrl(image) {
  return image.url;
}

// ../convert/src/serialize/serialize-image-shared/serialize-portive-image-url.ts
init_react_shim();
function serializePortiveImageUrl(image) {
  if (image.url.startsWith("$"))
    return "";
  const { hostname } = parseUrl2(image.url);
  if (hostname.match(/[.]portive[.]com$/i) && image.width && image.height)
    return `${image.url}?size=${image.width}x${image.height}`;
}

// ../convert/src/serialize/serialize-image-shared/serialize-uncommonmark-image-url.ts
init_react_shim();
...
// ../convert/src/serialize/serialize-line/index.ts
init_react_shim();

// ../convert/src/serialize/serialize-line/serialize-line.ts
init_react_shim();
import { Element as SlateElement2, Text as SlateText5 } from "slate";

// ../convert/src/serialize/serialize-line/diff-marks/index.ts
init_react_shim();

// ../convert/src/serialize/serialize-line/diff-marks/find-marks-to-add.ts
init_react_shim();
...
// ../convert/src/serialize/serialize-line/diff-marks/find-marks-to-remove.ts
init_react_shim();
...
// ../convert/src/serialize/serialize-line/normalize-line/index.ts
init_react_shim();
import { Element as Element26 } from "slate";

// ../convert/src/serialize/serialize-line/normalize-line/normalize-nodes.ts
init_react_shim();

// ../convert/src/serialize/serialize-line/normalize-line/run-normalizers-on-node.ts
init_react_shim();

// ../convert/src/serialize/serialize-line/normalize-line/normalizers/index.ts
init_react_shim();

// ../convert/src/serialize/serialize-line/normalize-line/normalizers/merge-adjacent-spaces.ts
init_react_shim();
...
// ../convert/src/serialize/serialize-line/normalize-line/normalizers/move-spaces-out-of-anchors.ts
init_react_shim();
...
// ../convert/src/serialize/serialize-line/normalize-line/normalizers/must-have-one-text-child.ts
init_react_shim();
...
// ../convert/src/serialize/serialize-line/normalize-line/normalizers/slice-spaces-at-node-boundaries.ts
init_react_shim();
...
// ../convert/src/serialize/serialize-line/normalize-line/normalizers/trim-spaces-at-end-of-line.ts
init_react_shim();
...
// ../convert/src/serialize/serialize-line/normalize-line/normalizers/trim-spaces-at-start-of-line.ts
init_react_shim();
...
var duplicateSegments = (segments) => {
  return segments.map((segment) => {
    if (Element26.isElement(segment) && segment.type === "anchor") {
      return {
        ...segment,
...
// ../convert/src/serialize/serialize-line/segment/serialize-segment.ts
init_react_shim();
import { Text as SlateText4 } from "slate";

// ../convert/src/serialize/serialize-line/segment/serialize-code-text.ts
init_react_shim();
...
// ../convert/src/serialize/serialize-line/segment/serialize-anchor.ts
init_react_shim();
...
// ../convert/src/serialize/serialize-line/segment/serialize-non-code-text.ts
init_react_shim();
...
function getNextMarks(segments, index2, trailingMarks) {
  for (let i = index2 + 1; i < segments.length; i++) {
    ...
    if (isPlainSpace(segment))
      continue;
    if (SlateElement2.isElement(segment) && segment.type === "image-inline")
      continue;
...
// ../convert/src/serialize/serialize-table/index.ts
init_react_shim();
...
`;
    ...
    case "task-list-item": {
      const indent3 = " ".repeat(element2.depth * LIST_INDENT_SIZE);
      return `${indent3}- [${element2.checked ? "x" : " "}] ${serializeLine(
        element2.children
      )}
...
import { createEditor, Editor as Editor57, Transforms as Transforms44 } from "slate";
...
function Editable2({ editor, onChange, ...extraProps }) {
  return /* @__PURE__ */ jsx66(
    Slate2,
    {
      ...
      onChange,
      children: /* @__PURE__ */ jsx66(SinkEditable2, { renderLeaf, ...extraProps })
    }
  );
}
...
function StandaloneEditor({
  ...
  return /* @__PURE__ */ jsx67(Editable2, { editor, onChange, placeholder });
}
function createWysimark(containerElement, options) {
  const editorRef = createRef();
  const root = createRoot(containerElement);
  root.render(/* @__PURE__ */ jsx67(StandaloneEditor, { editorRef, options }));
  return {
    unmount() {
      try {
        root.unmount();
    ...
    getMarkdown() {
      ...
      return typeof markdown === "string" ? markdown : options.initialMarkdown || "";
    },
